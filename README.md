...
使用 dsimage 替换模板：某某，把这个型号换进去
```

它就会按模板出一套图。风格模板按品牌规则生成；替换模板对着已有母版套图只换产品，未点名的字和版式不动，适合同一系列很多型号。

它和"再要一个 Prompt"的生图玩法有三个区别：

1. **做整套，不做单张** — 内置 26 种情景（主图、生活方式、平铺、模特、直播间、爆炸图、杂志大片……）。风格模板用 Campaign Style Lock 锁色板和光线；替换模板直接锁母版套图，换型号时只换货。
...
**1. 出图**  
把产品图丢过来，能给的信息一起给（品名、价格、尺寸、卖点、颜色），再说一句要什么，就可以开始。原图文件名会参与生图：一般就是型号，正面/背面等也会用来选参考图。缺的会先问一轮，你说先出也行。
...
**2. 同类品做模板**  
同一类货、同一套版式要反复出，就说「使用 dsimage，按这些参考图 / PDF 做一个模板」。还没有定稿套图 → 风格模板；已经有一套成品图、后面只换型号 → 替换模板（把那套图登记成母版）。就一份零散的，JSON 直接放 `templates/`，不必建文件夹。同一甲方多个品才放进同一个文件夹，语言、分辨率、格式、风格写在 `要求.json`。下次直接套。
...
输出按槽位命名（`h1.png`、`h2.png`…），进 `generated-images/backpack-pdp/`。Prompt 留在 `_prompts/backpack/`，不要放进品文件夹。替换模板把 `image` 写成数组：先母版路径，后产品图。个别槽位失败不影响其余槽位；修正后加 `--skip-existing` 重跑同一命令，只会补生成缺的图。
...
| `--image` | 参考图路径，可重复。替换模板：先母版后产品图 | 无 |
...
│       └── templates/
│           ├── _TEMPLATE_SPEC.md
│           ├── 01-默认电商模板.json  # 零散模板，不建文件夹
│           └── BeautyU/            # 同一甲方多个品才建文件夹
│               ├── 要求.json
│               ├── 风格/
│               └── 替换/
