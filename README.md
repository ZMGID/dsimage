...
Claude Code 只会从 `.claude/skills/` 目录发现 Skill。本仓库把 Skill 源文件放在 `skills/dsimage/`，使用前先复制过去：
...
# 或全局可用（所有项目都能触发）
cp -r skills/dsimage ~/.claude/skills/
```
...
```
![](docs/demo-claude-code.png)

![](docs/demo-amazon-pdp.png)
...
dsimage/
├── skills/                            # Skill 源文件
│   └── dsimage/                       # 复制到 .claude/skills/ 后即可被 Claude Code 发现
...
│       └── references/
│           └── templates/             # 25 个场景模板（01-hero-image ... 25-sports-campaign）
├── data/                              # 产品原图输入目录（自行创建，放入产品图）
├── docs/                              # 参考文档和演示截图
│   ├── apimart.md                     # apimart.ai GPT-Image-2 API 文档
