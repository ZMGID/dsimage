...
一个电商视觉创作 Skill，适用于 Claude Code / Codex / OpenClaw 等 AI 编程助手。给一张产品图、说一句需求，即可产出成套电商图片：商品主图、Amazon / Shopify 详情页、社媒推广图、直播间场景图等。

**核心能力：**

- **25 个场景模板** — 白底主图、生活方式、模特展示、直播间、爆炸图、杂志大片等，覆盖电商常见出图需求
- **Campaign Style Lock** — 多图任务自动锁定色板、光线、字体和背景，保证整套图风格统一
- **转化驱动** — 先诊断产品属于视觉 / 痛点 / 情感哪种购买驱动力，再按转化逻辑规划图片序列（Amazon PDP 默认 5 张主图 + 9 张详情页）
- **Prompt / 出图双模式** — 未配置 API 时输出视觉简报和 Prompt；配置任意 OpenAI 兼容图片 API 后直接出图
- **零依赖** — 生图脚本基于 Python 标准库，无需安装第三方包
