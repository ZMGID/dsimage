...
- 已点名「快速换货 / 同类快换 / 快速替换」且给了样板套图 + 产品图 → 读 `FAST_SWAP.md`。Agent 看图选白图、按模板出原型套图，改到点头再铺开。抽检默认关。档位、比例、交付由你按本轮填；生图比例必须对上目标画布。
...
**2. 一个大文件夹、下面很多编号夹，同时给了一套样板图**  
这不是「每个型号各自按规则新画一套」。正确做法是：按「甲方文件夹」拆成一品一夹，**同一套样板当母版，每个编号单独换一次货**。数量多、只换货、不改字 → 读 `FAST_SWAP.md`。问一句：是不是每个编号都套这套样板、只换货、字不动。
...
**2 个及以上品文件夹**：这一问只问一次。答案写入成图根 `_prompts/批次.json` 的 `only` / `skip` / `notes`。只换货、不改字 → `FAST_SWAP.md`（Agent 看图做原型，点头后再铺）。按规则画 → 「批量品目录」并发派工人。不要每个品再问一遍。
...
# 快速换货：Agent 看图选白图出原型，点头后再铺开（不要 --next）
...
| 成图 | `generated-images/`（单品）或同级 `{名}生成/`（批量，系列→生成） | 不要写回 `templates/` |
...
3. 按 SKILL「源图文件名」选该品产品图。`product_ref` 对不上（要背面没有背面图）→ 问用户或跳过该槽，禁止用正面硬贴背面页。快速换货不要走这步，走 `FAST_SWAP.md`：Agent 看图选白图，一品一张每槽都用。
...
9. 批量时：输出文件夹名用**商品编号**（见「甲方文件夹」），不要用英文 slug，也不要用号段夹名当一个品。

### 甲方文件夹（默认读存）

甲方给一个大文件夹。只读，不要改里面的文件。**这就是默认标准**，按规则画和快速换货都走这一套。用户另起成图根名才覆盖。

**读**

1. 大文件夹下面是编号夹。可能 **一个编号一个夹**（`V26025`），也可能 **几个编号一个夹**（`V26007-V26010`）。
2. 打开夹看图，按这个默认来（文件名**不带**「黑/米/红」，也基本没有「正面/背面」）：
   - **一品一张图**：这是常态。那张就是该品白图，也是换货用的产品参考。
   - **一夹多品**：夹里好几张、文件名带着不同商品编号。按文件名编号拆开，一品一套、一品仍是一张图。不要把整个号段夹当成一个品。
3. 夹名是号段、文件名却拆不开时停下问，不要猜着混出。不要靠文件名猜颜色。
4. 品的名字 = **商品编号**（`V26007`、`V26008`…），不是号段夹名，也不要改成英文 slug。

**存**

5. 在甲方大文件夹**同级**新建成图根：夹名以「系列」结尾则把「系列」换成「生成」（`VE男包系列` → `VE男包生成`）；否则在原名后加「生成」。不要建在甲方夹里面，不要用 `generated-images/` 顶替这批，不要再用 `{名}-成图`。
6. 成图根里 **一个商品编号一个文件夹**。号段夹要拆开写：`V26007/`、`V26008/`…，禁止输出一个 `V26007-V26010/`。
7. 每个编号夹里放两样：
   - 按模板槽位生成的套图（`h1.png`、`h2.png`…）
   - **一张白图**：从甲方该品源图迁过来（原文件名可保留）。就是做套图时选中的那张产品图，不是再生成一张。
8. Prompt / `jobs.json` 仍只放成图根 `_prompts/{商品编号}/`，不要和 `h1.png`、白图混在一个夹，也不要写进甲方源夹。

```text
1/
  VE男包系列/                 ← 甲方源（只读）
    V26007-V26010/            ← 一夹多品，文件名带编号
    V26025/                   ← 一品一夹，可能多色
    V26026/
  VE男包生成/                 ← 默认同级（系列→生成）
    V26007/                   ← 一品一夹
...
`--init --source` 直接指向甲方大文件夹。脚本按编号拆品（一品一张白图），拷进同级「生成」根。不要先手拆再 `--init`，也不要再套一层 `-成图`。不要从文件名猜颜色。

**完成：每个商品编号有自己的成图夹，里面有套图和一张白图；甲方源夹未被改写。**

### 落盘（成图 vs 提示词）

成图和提示词必须分开放。`prompt-H*.txt` 与 `jobs.json` **禁止**写入：甲方源夹、该品成图文件夹（不要和 `h1.png` 混放）、项目根、Skill 目录、`data/`。参考图给生图脚本用原路径；**白图按「甲方文件夹」拷进该品成图夹**，不要拷进 `_prompts/`。
...
**批量品目录**：读存见「甲方文件夹」。Prompt / `jobs.json` 放在成图根 `_prompts/{商品编号}/`。`output_dir` 写成 `../../{商品编号}`。

### 批量品目录（队列）

读存已经写在「甲方文件夹」。本节只写怎么排队出图。`--init --source` 直接给甲方大文件夹。

```text
VE男包系列/                 ← 甲方源（只读）
VE男包生成/                 ← 默认同级；一编号一夹 = 套图 + 白图
  V26007/h1.png
  V26007/白图
  _prompts/批次.json
```

1. 判定：用户给的路径下面有多个子文件夹、且子文件夹里是图 → 按「甲方文件夹」读存，再按本节排队。成图根默认同级「生成」（系列→生成）；用户另起名才用 `--output`。不要改甲方源夹。
2. 每个**商品编号**单独出一套。输出夹名是编号，不是号段夹名，不要加 `-pdp`。
3. 成图 `h1.png`… 和该品白图放进该编号夹。Prompt / `jobs.json` 放进 `_prompts/{编号}/`。已有同名成图夹就接着用，不要清空别人的夹。
...
- [ ] 快速换货已按 `FAST_SWAP.md`：Agent 看图选了白图；原型套图改到用户点头才铺开；铺开没有给每个品写 Prompt；出完才问要不要检查
...
- [ ] 甲方大文件夹已按「甲方文件夹」拆成一品一夹：一品一张白图；一夹多品按文件名编号拆开；成图在同级「生成」根、一编号一夹（套图 + 白图）；没有写进甲方源夹，没有把号段夹名当一个品，没有从文件名猜颜色
...
- [ ] Prompt / `jobs.json` 已写入 `_prompts/`（单品：`generated-images/_prompts/<slug>/`；批量：`{名}生成/_prompts/{编号}/`），没有写进甲方源夹、成图夹或项目根
...
| 快速换货却派品工人写长 Prompt | 读 FAST_SWAP.md；Agent 看图做原型，点头后再 --blast |
| 有母版却按情景重画 | 只换品；母版是锁；jobs.image = [母版, 产品图] |
| 未点名却改了母版上的字 | 政策 C：只有 editable_fields 且本轮给了新值才改 |
| 快速换货因缺「背面」文件名跳槽 | 一品一张白图，每槽都用这一张；不要跳 |
| 换货长 Prompt 缺角度用正面硬贴 | 问用户或跳过该槽 |
