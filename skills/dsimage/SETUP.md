...
2. **立刻**跑配置检查（不打接口、不扣费）。这一步把通道收明白：`--image`、参考图是否上送、本模式 `--size` 怎么翻译。通过之后这些就是出图时的既定事实，写进技能即可用。

```bash
# Windows 用 python，macOS/Linux 用 python3
python <技能目录>/scripts/gen_image.py --check
```

把 stdout 里的 `provider` / `mode` / 参考图上送方式用一句话告诉用户。失败则按报错改 `.env`，修好再 `--check`。

3. 询问用户是否再打一张**带参考图**的试图（会真实调用 API，产生少量费用，须先征得同意）：

```bash
python <技能目录>/scripts/gen_image.py --check --live
```

4. 成功 → 向用户报告生成文件路径，并说明：以后出套图按 SKILL 的命令加 `--run` 即可。失败 → 现在修（`401` = key 有误、`404` = 地址或模型名有误、超时 = 网络），修正后最多重试一次。
...
5. 运行 `python <已装>/scripts/check_scenes.py`，必须全部通过；不通过就修（常见问题：自建模板漏登记、其 pack 引用的自建情景还在）。已装目录有 `.env`（只说在/不在）时，再跑 `python <已装>/scripts/gen_image.py --check`（不要 `--live`，不要重配 API）。失败列入汇报。
6. 收尾汇报（短）：更新了哪几个已装目录、脚本报告里保留了什么（`.env` 只说「在/不在」，不回显内容）、合并了哪些沉淀、校验结果、`--check` 是否通过。不要让用户再选一次出图方式。
