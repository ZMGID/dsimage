...
2. 铺很多套（大文件夹）
一个大文件夹、每个子文件夹一个品。例如：「使用 dsimage 模板：箱包单品报价模板，把这个文件夹里每个子文件夹出一套」。要求只问一次，写进成图根 _prompts/批次.json。主会话只调度：每个品一个子代理写 Prompt；生图单独 queue_pack --run（默认 32 路）。不要在一条聊天里一个品做完再接下一个。
