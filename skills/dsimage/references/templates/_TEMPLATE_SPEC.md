...
- 位置：`references/templates/`，命名 `NN-kebab-case.json`，NN 用下一个可用序号（00 和本文件保留）
...
| ★ `name` | 必填 | 中文场景名，≤12 字 |
| ★ `keywords` | ≥5 个 | 中英文混合，用于语义匹配，**不得与其他模板重复** |
...
| ◐ `pack_structure` | 仅多图编排型模板（如 11-infographic） | 图片包的逐屏结构定义 |
...
```json
{
  ...
  "layout": "仅版式类模板需要（如网格/分屏/时间线）；摄影类省略",
  "angles": [
...
- 数值参照现有模板校准（如 01 主图 35-40%、02 场景图 20-25%）
...
- 不写该字段 = 用脚本默认（2k / png）；写了一个键就表示该模板有意覆盖默认
- 画幅 `--size` 不在本字段里，由 `default_ratio` 承担
- 优先级：用户命令行显式指定 > 模板 `generation` > 脚本默认
- 参考图 `--image`、输出目录 `--output-dir`、模式 `--mode` 是运行时信息，**禁止**写入模板
...
- 定义图片包：`conversion_drivers`（各驱动力序列）、`hero_pack` / `detail_pack`（张数、每屏 id/名称/信息图元素/角度/对应模板）、`font_pairing`、`rhythm`
