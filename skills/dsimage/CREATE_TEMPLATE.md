...
4. `pack` 草稿：槽位列表，每个槽位 = slot 编号 + 用途 + 比例 + **`overrides`（品牌对这张图的特化：构图占比/背景/布局/标注数量等，只写与所引情景默认的差异）**；用户要求几张就几个槽位。
...
**收尾汇报**：模板文件路径、匹配表登记情况、校验结果、试跑结论、修改模板的方法（直接改 JSON 再跑校验器）；并提醒用户——首批真实出图后按「坑跟谁走」回流翻车点（品牌对某张图的修正 → 该槽位 `overrides`；拍法通用坑 → 所引情景 pitfalls；法务/配色红线 → 模板 pitfalls）。
